...
function run(query, opts, cb) {
  ...
  if (arguments.length === 2) (cb = opts), (opts = {})
  ...
  let compiler = webpack({
    ...
    module: {
      rules: [{
        test: fixture,
        use: {
          loader: path.resolve(__dirname, '../src/index.js') + query,
          options: query ? undefined : opts,
        },
      }],
    },
  })
  ...
  compiler.run((err, stats) => {
    ...
    result = result
      .substr(result.indexOf(syncword) + syncword.length)
      .split('\n')

    result = result.slice(0, result.length - 2).join('\n')
